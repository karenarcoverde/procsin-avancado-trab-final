...
[ruido_puro, ~] = audioread('ruido_aviao.wav');   % Ruido puro

% Definir nova taxa de amostragem desejada
nova_fs = 48000;

% Aumentar a taxa de amostragem dos sinais de entrada
sinal_puro = resample(sinal_puro, nova_fs, fs);
ruido_puro = resample(ruido_puro, nova_fs, fs);
fs_antiga = fs; 
...
audiowrite('sinal_contaminado_ruido_aviao.wav', sinal_com_ruido, fs);

% Parâmetros da janela
tamanho_quadro = 100;           % Tamanho do quadro (número de amostras)
...
alpha1 = 0.99; %0.95 % Parâmetro para atualização rápida
alpha2 = 0.8; %0.8   % Parâmetro para atualização lenta
...
for n = 0:num_quadros-1
    ...
    if length(quadro) < tamanho_quadro
        z = zeros(1,(tamanho_quadro-length(quadro)));
        quadro = [quadro, z];
    end
...
audiowrite('sinal_limpo_ruido_aviao.wav', sinal_limpo, fs_antiga);

fs = fs_antiga; 
% Plotar os sinais para comparação (opcional)
...
subplot(4,2,4);
spectrogram(ruido_puro(1:length(sinal_com_ruido)), janela, sobreposicao, tamanho_quadro, fs, 'yaxis');
title('Espectrograma do ruído puro');
...
subplot(4,2,7);
plot(t, sinal_limpo(1:length(sinal_com_ruido)));
