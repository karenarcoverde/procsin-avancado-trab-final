...
# Gerar ruído branco
ruido_branco = np.random.normal(0, 1, len(sinal_voz))
ruido_branco = normalize_signal(ruido_branco)

# Certificar-se de que ambos os sinais têm o mesmo comprimento
min_len = min(len(sinal_voz), len(ruido_branco))
sinal_voz = sinal_voz[:min_len]
ruido_branco = ruido_branco[:min_len] / 8  # Ajustar a intensidade do ruído

# Sinal de voz com ruído branco
sinal_voz_ruido = sinal_voz + ruido_branco
...
plt.title('Sinal de voz + ruído branco')
plt.xlabel('Amostras')
plt.ylabel('Amplitude')

plt.subplot(3, 2, 4)
plot_spectrogram(sinal_voz_ruido, fs_voz, 'Espectrograma do sinal de voz + ruído branco')
